...
async function main() {
  ...
  if (password.length < 8) { console.log('Password too short. Aborted.'); rl.close(); return; }
  ...
  if (createSales.trim().toLowerCase() !== 'n') {
    ...
    if (salesPassword.length < 8) { console.log('Password too short, skipping sales user.'); salesUsername = null; }
  }
