...
// ═══════════════════════════════════════════════════════════
const bookings = {
  // Return all bookings with their rooms joined
  all() {
  ...
  // Returns bookings that have rooms conflicting with [startDate, endDate] for a given room number
  ...
  save: db.transaction(function(booking) {
    const {
      ...
      rooms, total, depositAmount, depositReceived, invoiceNum
    } = booking;
    ...
    if (existing) {
      db.prepare(`
        UPDATE bookings SET
          client_id=?, harvest_date=?, wet_weight=?, services=?,
          total=?, deposit_amount=?, deposit_received=?, invoice_num=?,
          updated_at=datetime('now')
        WHERE id=?
      `).run(
        clientId, harvestDate, wetWeight, JSON.stringify(services),
        total, depositAmount, depositReceived, invoiceNum, id
      );
      db.prepare('DELETE FROM booking_rooms WHERE booking_id = ?').run(id);
    } else {
      db.prepare(`
        INSERT INTO bookings
          (id, client_id, harvest_date, wet_weight, services, total, deposit_amount, deposit_received, invoice_num)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id, clientId, harvestDate, wetWeight, JSON.stringify(services),
        total, depositAmount, depositReceived, invoiceNum
      );
    }

    const insertRoom = db.prepare(`
      INSERT INTO booking_rooms (booking_id, room, start_date, end_date)
      VALUES (?, ?, ?, ?)
    `);
    for (const r of rooms) {
      insertRoom.run(id, r.room, r.startDate, r.endDate);
    }
